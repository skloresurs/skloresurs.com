...
export const env = createEnv({
  ...
  server: {
    ...
    GOOGLE_PRIVATE_KEY: z.string({ required_error: 'GOOGLE_PRIVATE_KEY is required' }).min(1),
    GOOGLE_SERVICE_ACCOUNT_EMAIL: z.string({ required_error: 'GOOGLE_SERVICE_ACCOUNT_EMAIL is required' }).email(),
    RECAPTCHA_SECRET: z.string({ required_error: 'RECAPTCHA_SECRET is required' }).min(1),
    SPREADSHEET_ID: z.string({ required_error: 'SPREADSHEET_ID is required' }).min(1),
    TELEGRAM_BOT_TOKEN: z.string({ required_error: 'TELEGRAM_BOT_TOKEN is required' }).min(1),
    TELEGRAM_CHAT_ID: z.string({ required_error: 'TELEGRAM_CHAT_ID is required' }).min(1),
  },
});
